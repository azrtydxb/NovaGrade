...
def test_grade_pdf_pipeline(monkeypatch, fake_client_factory, golden_transcript_dict, tmp_path):
    ...
    # first VLM call is the mark-map extraction (no stated total here), then transcription
    vlm = fake_client_factory([{"total": None, "sections": {}}, golden_transcript_dict["questions"]])
...
def test_grade_pdf_raises_on_empty_transcript(monkeypatch, fake_client_factory, tmp_path):
    ...
    # mark-map call, then a transcription that returns no questions
    vlm = fake_client_factory([{"total": None, "sections": {}}, []])
